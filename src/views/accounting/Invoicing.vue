...
onMounted(async () => {
  try {
    ...
    // If we have invoices, select the first one as default
    if (invoices.value.length > 0) {
      await selectInvoice(invoices.value[0].id);
    } else {
      // Use demo data if no invoices are available from API
      loadDemoData();
    }
  } catch (err) {
        ...
        <div v-else>
            <div class="flex items-start pt-6 px-6 pb-9 gap-6 flex-wrap-reverse">
                ...
                <div class="flex flex-col text-right">
                    ...
                    <span class="mt-1.5 body-medium">#{{ selectedInvoice?.id || '09022023' }}</span>
                </div>
            </div>
            <Divider />
            <div class="px-6 pb-9 pt-6 flex items-start gap-6 flex-wrap sm:flex-row flex-col">
                <div class="flex-1">
                    ...
                    <div class="mt-2 label-medium">{{ selectedInvoice?.customer?.company || 'AlphaHex' }}</div>
                    <div class="body-small text-left mt-0.5" v-html="selectedInvoice?.customer?.address || 'Claire Williams, 148 Hope LanePalo <br /> Alto, CA 94304.'"></div>
                </div>
                <div class="flex flex-col gap-3 min-w-64">
                    <div class="flex items-center justify-between gap-6">
                        <span class="body-small">Client Name</span>
                        <span class="label-small text-surface-950 dark:text-surface-0">{{ selectedInvoice?.customer?.name || 'Amy Elsner' }}</span>
                    </div>
                    <div class="flex items-center justify-between gap-6">
                        <span class="body-small">Date</span>
                        <span class="label-small text-surface-950 dark:text-surface-0">{{ selectedInvoice?.date || '04/19/2023' }}</span>
                    </div>
                    <div class="flex items-center justify-between gap-6">
                        <span class="body-small">Customer</span>
                        <span class="label-small text-surface-950 dark:text-surface-0">{{ selectedInvoice?.customer?.id || 'A123' }}</span>
                    </div>
                </div>
            </div>
            <Divider />
                ...
                <div class="py-6 px-4 flex items-start gap-6 flex-wrap sm:flex-row flex-col">
                    ...
                    <div class="flex flex-col gap-3 min-w-52">
                        <div class="flex items-center justify-between">
                            ...
                            <span>${{ selectedInvoice?.subtotal?.toFixed(2) || '216.00' }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <span>${{ selectedInvoice?.total?.toFixed(2) || '216.00' }}</span>
                        </div>
                    </div>
                </div>
            </div>
