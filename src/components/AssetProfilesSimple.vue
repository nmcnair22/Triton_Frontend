    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <Card v-for="profile in profiles" :key="profile.signature" class="hover:shadow-lg">
        ...
        <template #content>
          <!-- Asset Breakdown -->
          <div class="mb-3">
            <div class="text-sm text-gray-600 mb-1">Assets:</div>
            <div class="flex flex-wrap gap-2">
              <Tag v-for="(count, type) in profile.assetBreakdown" :key="type">
                {{ type }}: {{ count }}
              </Tag>
            </div>
          </div>
          ...
          <!-- Locations -->
          <div class="mb-3">
            <div class="text-sm text-gray-600">{{ profile.locationCount }} Locations</div>
          </div>
