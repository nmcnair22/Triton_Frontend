...
async function processExcelFile() {
  ...
  try {
    ...
    // If it's a data structure with filename and URL path
      ...
      props.file &&
      (props.file.fullPath || props.file.originalData?.path)
    ) {
...
<template>
  <div class="excel-preview">
    ...
    <div v-else class="excel-content">
      <!-- Sheet selector -->
      <div
        class="flex justify-content-between align-items-center p-3 border-bottom-1 surface-border mb-3"
      >
        <div class="flex align-items-center">
          ...
          <Select
            ...
            optionLabel="label"
            class="w-12rem"
            inputId="sheet-selector"
          />
        </div>
        <div>
          <Button
            v-tooltip.top="'Download Excel File'"
            icon="pi pi-download"
            text
            @click="downloadFile"
          />
