    ...
    <div v-else>
      <ul class="list-none p-0 m-0">
        ...
        >
          <div
            role="button"
            tabindex="0"
            class="w-full p-3 flex items-center justify-between text-left cursor-pointer border rounded-lg shadow-sm transition-all hover:shadow hover:bg-surface-50 dark:hover:bg-surface-800 focus:outline-none focus:ring-2 focus:ring-primary-400"
            :class="
              isTemplateSelected(template)
                ? 'bg-primary-50 dark:bg-primary-900/20 border-primary-500 dark:border-primary-400 ring-1 ring-primary-300 dark:ring-primary-600'
                : 'bg-surface-0 dark:bg-surface-900 border-surface-200 dark:border-surface-700'
            "
            :aria-pressed="isTemplateSelected(template)"
          ...
          >
            <div class="flex items-center min-w-0">
              <RadioButton
                :inputId="`invoice-template-${getTemplateId(template)}`"
                :modelValue="selectedTemplateId"
                :value="getTemplateId(template)"
                name="invoice-template"
                class="mr-3 shrink-0"
                @update:modelValue="selectTemplate(template)"
                @click.stop
              />
              <i
                class="pi pi-file-pdf text-2xl mr-3 text-primary-500 shrink-0"
                v-if="
              ...
              ></i>
              <i
                class="pi pi-file-excel text-2xl mr-3 text-green-500 shrink-0"
                v-else-if="
                  template.output_format === 'excel' ||
                  template.outputs?.includes('excel')
                "
              ></i>
              <i
                class="pi pi-file text-2xl mr-3 text-blue-500 shrink-0"
                v-else
              ></i>
              <div class="min-w-0">
                <div
                ...
                <div
                  class="text-surface-600 dark:text-surface-400 text-sm line-clamp-2"
                >
            ...
            <Tag
              v-if="template.type"
              :severity="getTemplateTypeBadge(template.type).severity"
              class="ml-3 shrink-0"
            >
      ...
      >
        ...
        <div v-if="useInvoiceDateOverride" class="space-y-3">
          ...
          <div
            v-else-if="invoiceDateOverride && isValidOverrideDate"
          ...
          <!-- Preview Information -->
          <div
            v-if="invoiceDateOverride && isValidOverrideDate"
          ...
          >
            <div class="text-sm text-blue-800 dark:text-blue-200">
              ...
              <div>
                Files will be named with
                <strong>{{ getOverrideDatePrefix() }}</strong> prefix
              </div>
          ...
          <div class="text-xs text-surface-600 dark:text-surface-400">
            ...
            Override the invoice date used for PDF display and file naming. Date
            must be between 2020-01-01 and {{ getCurrentYear() + 1 }}-12-31.
          </div>
        </div>
      </div>
      ...
      <div v-if="isMergeMode" class="mt-3">
        <div
          v-if="selectedInvoicesForMerge.length === 0"
        ...
        <div
          v-else-if="selectedInvoicesForMerge.length === 1"
          class="flex items-center gap-2 text-sm text-orange-600 dark:text-orange-400"
        >
          <i class="pi pi-exclamation-triangle"></i>
          <span>Select at least one more invoice to merge</span>
        </div>
        <div
          v-else-if="selectedInvoicesForMerge.length > 50"
          class="flex items-center gap-2 text-sm text-red-600 dark:text-red-400"
        >
          <i class="pi pi-times-circle"></i>
        ...
        >
          <i class="pi pi-check-circle"></i>
          <span
            >Ready to merge {{ selectedInvoicesForMerge.length }} invoices</span
          >
        </div>
      </div>
    </div>
...
const selectedTemplateId = computed(() => {
  return getTemplateId(props.selectedTemplate);
});
...
function getTemplateId(template) {
  return template?.id ?? template?.code ?? template?.template_id ?? "";
}
