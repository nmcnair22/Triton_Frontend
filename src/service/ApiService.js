...
function resolveApiBaseUrl() {
  ...
  const isLocalLaravelUrl =
    /^https?:\/\/(localhost|127\.0\.0\.1):8000\/api\/?$/i.test(
      configuredBaseUrl,
    );
...
export const ApiService = {
  ...
  get(resource, params) {
    ...
    return apiClient.get(cleanResource, { params }).catch((error) => {
  ...
  post(resource, data) {
    ...
    return apiClient.post(cleanResource, data).catch((error) => {
  ...
  put(resource, data) {
    ...
    return apiClient.put(cleanResource, data).catch((error) => {
  ...
  patch(resource, data) {
    ...
    return apiClient.patch(cleanResource, data).catch((error) => {
  ...
  delete(resource) {
    ...
    return apiClient.delete(cleanResource).catch((error) => {
...
export const VisitService = {
  ...
  assignTechnician(id, technicianId) {
    return ApiService.patch(`visits/${id}/assign-technician`, {
  ...
  batchUpdateStatus(visitIds, status) {
    return ApiService.patch("visits/batch/update-status", {
      visit_ids: visitIds,
      status,
    });
  },
...
export const InvoiceService = {
  ...
  getAvailableTemplates(clientId, invoiceNumber = null) {
    ...
    return ApiService.get("invoice-templates/available", params).catch(
      (error) => {
  ...
  generateTemplate(invoiceNumber, templateId, clientId, options = {}) {
    const postData = {
    ...
    return ApiService.post("invoice-templates/generate", postData)
      .then((response) => {
        ...
        if (response.data && response.status === 202) {
          ...
          if (!response.data.data) {
            response.data.data = {
              queued: true,
              job_id: response.data.job_id || "unknown",
            };
          }
        }
  ...
  getGeneratedFiles(invoiceNumber) {
    return ApiService.get(
      `invoice-templates/files/${encodeURIComponent(invoiceNumber)}`,
    );
  },
      ...
      if (parts.length >= 2) {
        ...
        if (type === "pdf" && parts.length > 2) {
          ...
          url = joinUrl(
            ...
            `invoice-templates/preview/${jobId}/${type}/${subtype}${tokenParam}`,
          );
        }
        // For Excel files with index
          ...
          url = joinUrl(
            apiClient.defaults.baseURL,
            `invoice-templates/preview/${jobId}/${type}/${index}${tokenParam}`,
          );
        } else {
          url = joinUrl(
            apiClient.defaults.baseURL,
            `invoice-templates/preview/${jobId}/${type}${tokenParam}`,
          );
        }
      } else {
        ...
        url = joinUrl(
          apiClient.defaults.baseURL,
          `invoice-templates/preview/${fileId}${tokenParam}`,
        );
      }
    } else {
      // Fallback to original behavior for compatibility
  ...
  getCustomerDocuments(customerNumber, limit = 50, offset = 0) {
    return ApiService.get("invoice-templates/customer-documents", {
  ...
  getInvoiceDocuments(invoiceNumber) {
    return ApiService.get("invoice-templates/invoice-documents", {
      invoice_number: invoiceNumber,
    });
  },
};
